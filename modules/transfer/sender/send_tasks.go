...
func startSendTasks() {
	...
	for node := range cfg.Judge.Cluster {
		queue := EJudgeQueues[node]
		go forward2EJudgeTask(queue, node, judgeConcurrent)
	}
...
func forward2JudgeTask(Q *list.SafeListLimited, node string, concurrent int) {
	cfg := g.Config()
	batch := cfg.Judge.Batch // 一次发送,最多batch条数据
	addr := cfg.Judge.Cluster[node]
...
// Judge定时任务, 将 Judge发送缓存中的数据 通过rpc连接池 发送到Judge
func forward2EJudgeTask(Q *list.SafeListLimited, node string, concurrent int) {
	...
	for {
		...
		judgeItems := make([]*cmodel.EMetric, count)
		for i := 0; i < count; i++ {
			judgeItems[i] = items[i].(*cmodel.EMetric)
		}

		//	同步Call + 有限并发 进行发送
		sema.Acquire()
		go func(addr string, judgeItems []*cmodel.EMetric, count int) {
			defer sema.Release()
			...
			sendOk := false
			for i := 0; i < 3; i++ { //最多重试3次
				err = EJudgeConnPools.Call(addr, "Judge.SendE", judgeItems, resp)
				if err == nil {
					sendOk = true
					break
				}
			...
			if !sendOk {
				log.Printf("send judge %s:%s fail: %v", node, addr, err)
				proc.SendToEJudgeFailCnt.IncrBy(int64(count))
			} else {
				proc.SendToEJudgeCnt.IncrBy(int64(count))
			}
		}(addr, judgeItems, count)
	}
}
